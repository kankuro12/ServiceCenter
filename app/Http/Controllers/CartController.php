    ...
    public function productAddToCart(Request $request){
        ...
        if($productCount>0){
            if($request->has('color_id')){
                $CartProduct = Cart::where('session_id',$session_id)->where('product_id',$request->product_id)->where('color_id',$request->color_id)->where('size_id',$request->size_id)->first();
                $stockChceckCount = Stock::where('product_id',$request->product_id)->where('color_id',$request->color_id)->where('size_id',$request->size_id)->count();
                if($stockChceckCount>0){
                    $stockChceck = Stock::where('product_id',$request->product_id)->where('color_id',$request->color_id)->where('size_id',$request->size_id)->first();
                       $requestQty = $CartProduct->qty + $qty;
                    if($stockChceck->total>=$requestQty){
                        $cartQty = $CartProduct->qty;
                        $CartProduct->qty = $cartQty + 1;
                        $CartProduct->save();
                    }else{
                       return redirect()->back()->with('warning','This product stock quantity is only '.$stockChceck->total.' !');
                    }
                }else{
                   return redirect()->back()->with('warning','This product is out of stock !');
                }
            }else{
                $cartProduct = Cart::where('session_id',$session_id)->where('product_id',$request->product_id)->first();
                $simpleStockCount = Simplestock::where('product_id',$request->product_id)->count();
                if($simpleStockCount>0){
                    $simpleStock = Simplestock::where('product_id',$request->product_id)->first();
                    $requestQty = $cartProduct->qty + $qty;
                    if($simpleStock->total>=$requestQty){
                        $cartProduct->qty = $cartProduct->qty + 1;
                        $cartProduct->save();
                    }else{
                        return redirect()->back()->with('warning','This product stock quantity is only '.$simpleStock->total.' !');
                    }
                }else{
                   return redirect()->back()->with('warning','This product is out of stock !');
                }
            ...
            if($request->has('size_id')){
                $stock_price = Stock::where('product_id',$request->product_id)->where('color_id',$request->color_id)->where('size_id',$request->size_id)->first();
                $size_price = $stock_price->saleprice;
                $cartData->price = $size_price;
            }else{
