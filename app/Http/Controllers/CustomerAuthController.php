...
class CustomerAuthController extends Controller
{
    ...
    public function customerLoginRegister(Request $request)
    {
        ...
        if ($customerCount > 0) {
            ...
            $cus->save();
            Mail::to($cus)->send(new \App\Mail\CutomerWelcome($cus));
    ...
    public function checkout()
    {
        ...
        if($cartCount>0){
            ...
            return redirect('shopping-cart')->with('warning','You do not have any items in your cart!');
        }
    }
    ...
    public function placeOrderStore(Request $request)
    {
        ...
        Mail::to($request->user())->send(new \App\Mail\OrderPlaced($arr));
    ...
    public function forgetPassword(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            if ($user > 0) {
                ...
                Mail::to($cus)->send(new \App\Mail\ResetPass($str));
    ...
    public function changePassword(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            $cats = Category::with('subcat')->where('parent_id', 0)->get();
            $code = $request->code ?? '';
            return view('front.customerAuth.change_pass', compact('cats', 'code'));
        }
    }
}
