...
@section('js')
    <script>
        ...
        $(document).ready(function() {
            ...
            if (ismobile) {
                cats.forEach(cat => {
                    catele.innerHTML += '<li class="single-cat" id="single-cat-' + cat.i + '" data-id="' +
                        cat.i + '" data-name="' + cat.n + '">' +
                ...
                cats.forEach(cat => {
                    catele.innerHTML += '<li class="single-cat" id="single-cat-' + cat.i + '" data-id="' +
                        cat.i + '" data-name="' + cat.n + '">' + cat.n + '</li>';
                    html = '<div class="row d-none cat" id="cats-' + cat.i + '">';
                    cat.j.forEach(job => {
                        let d = job.sdata.split("|");
                        html += '<div class="col-md-4  " id="cat-' + d[0] + '">' +
                            '<a href="' + (url.replace('xxx_id', d[0])) +
                            '" class="single-cat-view  ">' +
                            '<div class="job-name nowrap">' + d[1] + '</div>' +
                            '<div class="job-company nowrap">' + d[3] + '</div>' +
                            '<div class="job-duedate"> <span>Due Date</span><span>' + d[2] +
                            '</span></div>' +
                            '</a>' +
                            '</div>';
                    });
                    html += "<div class='py-2'><a class='btn btn-primary' href='" + (caturl.replace(
                        'xxx_id', cat.i)) + "'>View More In " + cat.n + "</a></div></div>";
                    // console.log(html);
