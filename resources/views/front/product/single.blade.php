    ...
    <div class="page-content">
        <div class="container">
                            ...
                            <div class="product-gallery product-gallery-vertical">
                                <div class="row">
                                    <figure class="product-main-image">
                                        <img id="product-zoom"
                                            src="{{ asset('back/images/product/'.$product->feature_image) }}"
                                            data-zoom-image="{{ asset('back/images/product/'.$product->feature_image) }}"
                                            alt="product image">
                                    ...
                                    <div id="product-zoom-gallery" class="product-image-gallery">
                                        <a class="product-gallery-item active" href="#"
                                            data-image="{{ asset('back/images/product/'.$product->feature_image) }}"
                                            data-zoom-image="{{ asset('back/images/product/'.$product->feature_image) }}">
                                            <img src="{{ asset('back/images/product/'.$product->feature_image) }}"
                                                alt="product side">
                                        </a>
                                        @foreach ($g_image as $i)
                                        <a class="product-gallery-item" href="#"
                                            data-image="{{ asset('back/images/gallery/'.$i->image) }}"
                                            data-zoom-image="{{ asset('back/images/gallery/'.$i->image) }}">
                            ...
                            <div class="product-details product-details-centered">
                                ...
                                <div class="product-price">
                                    ...
                                    @if ($product->sales_price < $product->price)
                                        <span class="font-weight-light line-through" id="oldprice">NPR
                                            {{ $product->price }}</span>
                                        @endif
                                ...
                                <div class="details-filter-row details-row-size">
                                    ...
                                    <div class="select-custom">
                                        <select name="color_id" class="form-control" id="color" required>
                                            ...
                                            <option value="{{ $item->id }}">{{ $item->name }} ({{ $item->shortcode }})
                                            </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="product-details-action">
                                    <div class="details-action-col">
                                        ...
                                        <div class="product-details-quantity">
                                            ...
                                            <input type="number" id="qty" name="qty" class="form-control" value="1"
                                                min="1" max="{{ $stockCheck->total }}" step="1" data-decimals="0"
                                                required>
                                            @else
                                            <input type="number" id="qty" name="qty" class="form-control" value="1"
                                                min="1" max="5" step="1" data-decimals="0" required>
                                            @endif
                                    ...
                                    <div class="details-action-wrapper">
                                        <a href="{{ url('add-to-withlist',$product->id) }}"
                                            class="btn-product btn-wishlist text-white" title="Add to wishlist"
                                            style="background: #E88C36; border:1px solid #E88C36; padding:4px;"><span
                                                class="text-white">Add to wishlist</span></a>
                                        <span class="btn-product btn-wishlist text-white" title="Add to wishlist"
                                            style="visibility: hidden;"><span class="text-white">add to
                                                wishlist</span></span>
                                ...
                                <div class="product-details-footer">
                                    <div class="product-cat">
                                        <span>Category:</span>
                                        <a
                                            href="{{ url('shops-by-category',$product->category->id) }}">{{ $product->category->name }}</a>,
            ...
            <div class="owl-carousel owl-simple carousel-equal-height carousel-with-shadow" data-toggle="owl"
                ...
                @php
                $productByCat =
                \App\Product::where('category_id',$product->category->id)->inRandomOrder()->take(10)->get();
                @endphp
                ...
                <form action="{{ url('add-to-cart') }}" method="POST">
                    ...
                    <div class="product product-7 text-center">
                            ...
                            @if($item->sales_price < $item->price)
                                ...
                                @if ($current_date < $item->newProduct())
                                    ...
                                    <div class="product-action"
                                        ...
                                        @if ($stockCount>0)
                                        @if($stockCheck->total>0)
                                        <button class="btn-product btn-cart" title="Add to cart"
                                            style="border: 0;"><span>add to cart</span></button>
                                        @else
                                        ...
                                        @endif
                                        @else
                                        <a href="{{ url('/product/detail/'.$item->id) }}" class="btn-product btn-cart"
                                            title="View Detail"><span>View Detail</span></a>
                                        @endif
                        ...
                        <div class="product-body">
                            <div class="product-cat">
                                <a
                                    href="{{ url('shops-by-category',$item->category->id)}}">{{ $item->category->name }}</a>
                            </div><!-- End .product-cat -->
...
<script>
  ...
  $('#color').on('change', function(e){
    ...
    axios.get('/size/by-color/'+color_id+'/{{$product->id}}')
 ...
 // get priceBy size
  $('#productSize').on('change', function(e){
    ...
    axios.get('/price/by-size/'+color_id+'/'+size_id+'/{{$product->id}}')
      .then(function (response) {
          var data = response.data;
          $('#productPrice').empty();
          $('#productPrice').text('NPR '+data.saleprice+'.00');
          if(data.price == data.saleprice){
            $('#oldprice').empty();
          }else{
            $('#oldprice').empty();
            $('#oldprice').text('NPR '+data.price+'.00');
          }
      console.log(data);
    })
    .catch(function (error) {
